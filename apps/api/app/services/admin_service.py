...
def dispatch_daily_monitor() -> dict[str, str]:
    ...
    watchlist = app.send_task("app.tasks.collect_watchlist.collect_watchlist_albo")
    return {
        "official_task_id": str(official.id),
        "eproc_task_id": str(eproc.id),
        "watchlist_task_id": str(watchlist.id),
    }


def dispatch_watchlist_scan() -> str:
    app = _celery_app()
    result = app.send_task("app.tasks.collect_watchlist.collect_watchlist_albo")
